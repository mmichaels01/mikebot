...
def PlayR2Sound(sound):
    cmd = 'mpg321 /home/pi/sounds/r2_'+sound+'.mp3 2> /dev/null &'
    os.system(cmd)
    return
...
try:
    ...
    while True:
      ...
      # rightDistanceInches = Reading(TRIG1, ECHO1)
        ...
        if(abs(leftDistanceInches - lastLeft)> 10 or abs(forwardDistanceInches - lastForward) > 10 ):
            print("erroneous reading? continuing one more update")
            leftDistanceInches = lastLeft
            forwardDistanceInches = lastForward
        #if(forwardDistanceInches < 1 or leftDistanceInches < 1 or rightDistanceInches < 1):
        if(forwardDistanceInches < 1 or leftDistanceInches < 1):
            print ("What")
            break
        if(abs(LEFT_INITIAL - leftDistanceInches) > 1):
            if(FORWARD_INITIAL - forwardDistanceInches > 1):
                print("turning left")
                TIME_ON_LEFT = .03
            if(FORWARD_INITIAL - forwardDistanceInches < -1):
                #Turning right
